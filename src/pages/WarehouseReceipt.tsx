...
import { receiptHeader as initHeader, receiptItems as initItems, receiptFooter as initFooter, COLUMNS } from '../data/fakeData';
import { type ReceiptHeader, type ReceiptItem, type ReceiptFooter } from '../types';

const computeFooter = (items: ReceiptItem[], base: ReceiptFooter): ReceiptFooter => {
  const totalQuantity = items.reduce((s, i) => s + i.quantity, 0);
  const totalAmount = items.reduce((s, i) => s + i.totalPrice, 0);
  const totalWeight = items.reduce((s, i) => s + i.weight1 * i.quantity, 0);
  return { ...base, totalQuantity, totalAmount, totalWeight: Math.round(totalWeight * 100) / 100 };
};
...
const WarehouseReceipt: FC = () => {
  ...
  const [footer, setFooter] = useState<ReceiptFooter>(computeFooter(initItems, initFooter));
  ...
  const handleHeaderChange = useCallback((field: keyof ReceiptHeader, value: string) => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100 overflow-hidden" style={{ minWidth: 900 }}>
      {contextHolder}

      {/* Window title bar */}
      <div
        className="flex items-center justify-between bg-gradient-to-b from-[#1c5c9e] to-[#1a4f8a] px-3 py-1.5 text-white select-none"
        dir="rtl"
      >
