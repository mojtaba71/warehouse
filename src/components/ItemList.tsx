...
const LookupModal: FC<LookupModalProps> = ({
  lookupType,
  onSelect,
  onClose,
}) => {
      ...
      >
        ...
        >
          <div
            style={{
              padding: "8px 10px",
              borderBottom: "1px solid #dbeaf8",
              background: "#eef4fb",
            }}
          >
            <span style={{ fontSize: 12, fontWeight: 600, color: "#1a4f82" }}>
              انتخاب کالا
            </span>
          </div>
          <div
            style={{ padding: "6px 10px", borderBottom: "1px solid #dbeaf8" }}
          >
            <input
              ref={searchRef}
              value={search}
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            ...
            >
              <thead>
                <tr
                  style={{ background: "#dbeaf8", position: "sticky", top: 0 }}
                >
                  <th
                    style={{
                      padding: "4px 8px",
              ...
              <tbody>
                {filtered.map((p, i) => (
                  <tr
                    ...
                    onMouseEnter={(e) =>
                      (e.currentTarget.style.background = "#dbeaf8")
                    }
                    onMouseLeave={(e) =>
  ...
  const opts =
    lookupType === "deliverySource"
      ? deliverySourceOptions
      : deliveryDestinationOptions;
  const filtered = opts.filter(
    (o) => o.name.includes(search) || o.code.includes(search),
  );
  const label =
    lookupType === "deliverySource"
      ? "انتخاب تحویل گیرنده"
...
const ItemList: FC<Props> = ({
  ...
  const [lookupCell, setLookupCell] = useState<{
    id: number;
    field: keyof ReceiptItem;
    type: LookupType;
  } | null>(null);
  ...
  const SortIcon: FC<{ field: keyof ReceiptItem }> = ({ field }) => {
    if (sort.field !== field)
      return <MdUnfoldMore size={12} className="opacity-40" />;
    return sort.dir === "asc" ? (
  ...
  return (
    <>
      <div
        className="flex-1 overflow-auto border-b border-gray-300 bg-white"
        style={{ direction: "rtl", minHeight: 0 }}
      >
        {/* Header */}
        <div
          className="flex sticky top-0 z-10 bg-linear-to-b from-gray-200 to-gray-300 border-b-2 border-gray-400 select-none"
        ...
        <div style={{ minWidth: columns.reduce((s, c) => s + c.width, 0) }}>
          {sorted.map((item, rowIdx) => {
            ...
            return (
              ...
              >
                {columns.map((col) => {
                  const isEditing =
                    editCell?.id === item.id && editCell?.field === col.key;
                  const isFocused =
                    focusCell?.id === item.id && focusCell?.field === col.key;
                  ...
                  return (
                    <div
                      ...
                      onClick={(e) => {
                        ...
                        onSelect(item.id);
                        if (col.editable)
                          setFocusCell({ id: item.id, field: col.key });
                      }}
                      onDoubleClick={() => {
                        if (col.editable && !col.lookup)
                          startEdit(item.id, col.key, item[col.key]);
                      }}
                    >
                      {isEditing ? (
                        <input
                          ...
                          style={{
                            direction: col.align === "left" ? "ltr" : "rtl",
                            textAlign: col.align,
                          }}
          ...
          {/* Empty rows */}
          {Array.from({ length: Math.max(0, 8 - sorted.length) }).map(
            (_, i) => (
              <div
                key={`empty-${i}`}
